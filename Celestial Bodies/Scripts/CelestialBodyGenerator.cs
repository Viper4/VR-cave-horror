...
public class CelestialBodyGenerator : MonoBehaviour
{
    ...
    [SerializeField] float quadVisibleAngle = 90;
    [SerializeField] float quadSplitDistance = 50;
    [SerializeField] bool autoUpdateQuads;
    ...
    private void Init()
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            for (int r = 0; r < rootLOD; r++)
            {
                ...
                {
                    rootChunks[arrayIndex] = new TerrainChunk(shapeGenerator, shapeSettings, directions[i], r, c, rootLOD, quadSplitDistance, quadVisibleAngle);
